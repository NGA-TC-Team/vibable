...
export function ScreenDesignForm({ disabled = false }: { disabled?: boolean }) {
  ...
  const removePage = (index: number) => {
    patchData({ pages: data.pages.filter((_, i) => i !== index) });
  };
  ...
  const addError = (pageIndex: number) => {
    ...
    pages[pageIndex] = {
      ...pages[pageIndex],
      states: {
        ...pages[pageIndex].states,
        errors: [...pages[pageIndex].states.errors, err],
      },
    };
    patchData({ pages });
  };
  ...
  const updateError = (
    ...
    pages[pageIndex] = {
      ...pages[pageIndex],
      states: { ...pages[pageIndex].states, errors },
    };
    patchData({ pages });
  };
  ...
  const removeError = (pageIndex: number, errIndex: number) => {
    ...
    pages[pageIndex] = {
      ...
      states: {
        ...pages[pageIndex].states,
        errors: pages[pageIndex].states.errors.filter(
          (_, i) => i !== errIndex,
        ),
      },
    };
  ...
  const addInteraction = (pageIndex: number) => {
    ...
    pages[pageIndex] = {
      ...pages[pageIndex],
      interactions: [...pages[pageIndex].interactions, interaction],
    };
    patchData({ pages });
  };
  ...
  const removeInteraction = (pageIndex: number, intIndex: number) => {
    ...
    pages[pageIndex] = {
      ...pages[pageIndex],
      interactions: pages[pageIndex].interactions.filter(
        (_, i) => i !== intIndex,
      ),
    };
    patchData({ pages });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <Label className="text-base font-semibold">화면 목록</Label>
        {!disabled && (
      ...
      {data.pages.map((page, pi) => (
        <div key={page.id} className="rounded-lg border p-3 space-y-3">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                updatePage(pi, {
                  states: { ...page.states, idle: e.target.value },
                })
              }
              disabled={disabled}
            />
            <Input
              ...
              onChange={(e) =>
                updatePage(pi, {
                  states: { ...page.states, loading: e.target.value },
                })
              }
              disabled={disabled}
            />
            <Input
              ...
              onChange={(e) =>
                updatePage(pi, {
                  states: { ...page.states, offline: e.target.value },
                })
              }
              disabled={disabled}
            />
            ...
            {page.states.errors.map((err, ei) => (
              <div key={ei} className="flex gap-2">
                <Select
                  value={err.type}
                  onValueChange={(v) =>
                    updateError(pi, ei, { type: v as ErrorState["type"] })
                  }
                  disabled={disabled}
                >
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateError(pi, ei, { description: e.target.value })
                  }
                  disabled={disabled}
                />
                {!disabled && (
            ...
            {page.interactions.map((int, ii) => (
              <div key={ii} className="grid grid-cols-3 gap-2">
                <Input
                  ...
                  value={int.element}
                  onChange={(e) =>
                    updateInteraction(pi, ii, { element: e.target.value })
                  }
                  disabled={disabled}
                />
                <Input
                  ...
                  value={int.trigger}
                  onChange={(e) =>
                    updateInteraction(pi, ii, { trigger: e.target.value })
                  }
                  disabled={disabled}
                />
                <div className="flex gap-1">
                  <Input
                    ...
                    value={int.action}
                    onChange={(e) =>
                      updateInteraction(pi, ii, { action: e.target.value })
                    }
                    disabled={disabled}
                  />
                  {!disabled && (
